    ...
    data.forEach((country) => {
      ...
      let cardContent = `
        <div class="card justify-content-center">
          <div class="card-header text-center badge bg-dark text-wrap">${
            country.name.common
          }</div>
          <div class="card-body justify-content-center">
           <img src="${country.flags.svg}" class="card-img-top" alt="Flag">
            ...
            <button type="button" class="btn btn-outline-dark justify-content-center  badge bg text-wrap" 
             onclick="getWeather('${
               country.name.common
...
const Weather = "https://openweathermap.org";
function getWeather(country) {
  const response = fetch(
    `https://api.openweathermap.org/data/2.5/weather?q=${country}&appid=${Weather}`
  );
  console.log("API KEY:", Weather);
